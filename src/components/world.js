...
class World extends Preact.Component {
  ...
  componentDidMount() {
    ...
    svg.append("use").attr("class", styles.fill).attr("xlink:href", "#sphere");
    ...
    const midPoint = svg
      ...
      .attr("d", function(d) {
        return path(d.features[3].geometry);
      });
    ...
    function position_labels() {
      svg
        ...
        .attr("transform", function(d) {
          return (
            "translate(" + projection(d.features[3].geometry.coordinates) + ")"
          );
        })
        .attr("dx", function(d) {
        ...
        .attr("opacity", function(d) {
          var geoangle = geo.geoDistance(d.features[3].geometry.coordinates, [
            -projection.rotate()[0],
            projection.rotate()[1]
          ]);
    ...
    timer.interval(function() {
      ...
      widePathGroup.attr("d", function(d) {
        return path(d.features[1].geometry);
      });
      midPoint.attr("d", function(d) {
        return path(d.features[3].geometry);
      });
  ...
  render() {
    return (
      <div
        ...
        aria-label="Spinning globe of world showing the path of the next 5 total solar eclipses over populated parts of the world"
      >
